                        ...
                        <div class="mb-4">
                            ...
                            <select name="category_id" id="category_id"
                                ...
                                @foreach($categories as $category)
                                    <option
                                        value="{{ $category->id }}" {{ $card->category_id == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                                @endforeach
                            </select>
                        </div>
                                ...
                                @foreach($seasons as $season)
                                    <div class="flex items-center">
                                        <input type="checkbox" name="seasons[]" id="season_{{ $season->id }}"
                                            ...
                                            {{ $card->seasons->contains($season->id) ? 'checked' : '' }}>
                        ...
                        <div class="mb-4">
                            ...
                            @if($card->image_url)
                                <img src="{{ asset('storage/' . $card->image_url) }}" alt="{{ $card->title }}"
                                     class="h-20 w-20 rounded-md object-cover mb-2">
                            @else
                                <p class="text-gray-500">No image uploaded.</p>
                            @endif
                            <label for="image" class="block text-sm font-medium text-gray-700">New Image
                                (optional)</label>
